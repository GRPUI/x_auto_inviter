...
async def get_x_link(users_to_invite: List[str], community: str, token: str, invited_users_key: str):
    ...
    async with AsyncCamoufox(headless=True, humanize=1) as browser:
        ...
        await page.click("button.r-1wron08:nth-child(2) > div:nth-child(1) > svg:nth-child(1)")
        await page.click("#layers > div.css-175oi2r.r-zchlnj.r-1d2f490.r-u8s1d.r-ipm5af.r-1p0dtai.r-105ug2t > div > div > div > div.css-175oi2r.r-1ny4l3l > div > div.css-175oi2r.r-j2cz3j.r-kemksi.r-1q9bdsx.r-qo02w8.r-1udh08x.r-u8s1d > div > div > div > a > div.css-175oi2r.r-16y2uox.r-1wbh5a2 > div > span")
        ...
        for user in users_to_invite:
            ...
            async with lock:
                if not lock.acquired:
                    logger.debug(f"User {user} is being invited by another worker, skipping")
                    continue
...
async def worker(worker_id: int, token_queue: asyncio.Queue, users_to_invite: List[str], community: str, invited_users_key: str):
    ...
    while True:
        try:
            ...
            try:
                await get_x_link(users_to_invite=users_to_invite, community=community, token=token, invited_users_key=invited_users_key)
                logger.info(f"[Worker {worker_id}] Completed token {index}")
            except Exception as e:
                logger.error(f"[Worker {worker_id}] Error processing token {index}: {e}")
            finally:
                token_queue.task_done()
        except Exception as e:
...
async def main_cli(tokens_file: str, users_file: str, workers: int, community: str = "https://x.com/i/communities/1996945882479026553/", redis_url: str = "redis://localhost:6379"):
...
if __name__ == "__main__":
    ...
    try:
        asyncio.run(main_cli(args.tokens, args.users, args.workers, args.community, args.redis))
